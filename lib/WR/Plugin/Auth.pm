...
sub register {
    ...
    $app->helper(current_user_cids => sub {
        my $self = shift;
        my $cids = [];

        push(@$cids, $self->current_user->{_id});
        push(@$cids, @{$self->current_user->{ucid}}) if(defined($self->current_user->{ucid}) && ref($self->current_user->{ucid}) eq 'ARRAY');
        return $cids;
    });
    
    $app->helper(load_user_data => sub {
        my $self = shift;
        my $id   = shift;
        my $cb   = shift;
