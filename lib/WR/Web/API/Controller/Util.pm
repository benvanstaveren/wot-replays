package WR::Web::API::Controller::Util;
...
# WOT mod battle result submission
sub battleresult_submit {
    ...
    if(defined($data->{arena_id}) && defined($data->{battleResult})) {
        ...
        if(defined($br)) {
            ...
            $self->render(text => 'DATA:OK BR:FAIL SAVE:FAIL', status => 200);
        }
    } else {
        $self->render(text => 'DATA:FAIL BR:FAIL SAVE:FAIL', status => 200);
    }
}
